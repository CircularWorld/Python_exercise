...
for item in IterableHelper.find_all(list_employee, lambda item: item.money >= 15000):
    print(item.__dict__)

# for item in find_all(condition01):
#     print(item.__dict__)
print()
for item in IterableHelper.find_all(list_employee, lambda item: item.did == 9005):
    print(item.__dict__)
