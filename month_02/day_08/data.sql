...
---4
select student.sid,sname,avg(number) from student inner join score on student.sid = score.student_id group by sid having avg(number) <85;

select student.sid,sname,number from student left join score on student.sid = score.student_id where score.course_id =2;

select cname,count(cname) from student left join course on student.class_id = course.cid  group by student.class_id;
