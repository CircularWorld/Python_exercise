...
def main():
    ...
    while (ctrl):
        ...
        option_dict = {'1':insert,'2':search,'3':delete,'4':modify,'5':sort,'6':total,'7':show}
...
def show():
    ...
    if os.path.exists(filename):  # 判断文件是否存在
        with open(filename, 'r') as rfile:  # 打开文件
            student_str = rfile.readlines()  # 读取全部内容
        for list in student_str:
            student_dict.append(eval(list))  # 将找到的学生信息保存到列表中
        if student_dict:
